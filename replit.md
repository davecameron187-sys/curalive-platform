...
- **Downloads**: Event detail panel includes a Downloads section with two tabs:
  - **Recording** — Download MP3 (green, active when `recording_path` exists in archive_events). Files served from `uploads/recordings/` with path traversal protection.
  - **Transcription** — Download TXT (blue, always active when transcript_text exists). Includes CuraLive header with event metadata.
