...
### AI Intelligence Services (Board/Pre-Event/Compliance)
- **Board Intelligence Compass** (`boardIntelligenceRouter.ts`, `BoardIntelligenceCompass.tsx`): Pre-board briefing pack with prior commitment audit, director liability mapping, analyst expectation audit, governance communication scoring, and board resolutions/action items. Accessible via Shadow Mode "Board Intelligence" tab.
- **Pre-Event Intelligence Briefing** (`preEventIntelligenceRouter.ts`, `PreEventIntelligenceBriefing.tsx`): 60-minute prep briefing with analyst consensus data, predicted Q&A items with probability scores, compliance hotspots, and event readiness scores by category. Accessible via Shadow Mode "Pre-Event Briefing" tab.
- **Regulatory Compliance Monitor** (`regulatoryComplianceRouter.ts`, `RegulatoryComplianceMonitor.tsx`): Real-time jurisdiction-aware compliance flagging with regulatory flags (SEC/FCA/ASIC/SGX/HKEX/JSE), disclosure trigger detection, jurisdiction profile management, and compliance action items. Pre-seeded with 6 jurisdiction profiles. Accessible via Shadow Mode "Compliance Monitor" tab.
- **DB Tables**: `board_intelligence_compass`, `prior_commitment_audits`, `director_liability_maps`, `analyst_expectation_audits`, `governance_communication_scores`, `board_resolutions`, `compass_provenance`, `compass_action_history`, `pre_event_intelligence_briefings`, `analyst_consensus_data`, `predicted_qa_items`, `compliance_hotspots`, `readiness_scores`, `briefing_provenance`, `briefing_action_history`, `regulatory_compliance_monitors`, `regulatory_flags`, `disclosure_triggers`, `jurisdiction_profiles`, `compliance_action_items`, `compliance_provenance`.
