...
- **Transcript-synced recording playback** — click-to-seek and active segment highlighting
- **Export hardening** — CSV/JSON completeness, formula injection protection, compliance field guarantees
- **PDF export** — formatted printable report with all session data and compliance sections
...
- Strips newlines (replaces with spaces)
- Detects formula-trigger prefixes (=, +, -, @, tab, carriage return)
- Prepends a single quote to neutralize formula execution in spreadsheets
- Double-quotes all values and escapes internal quotes
...
### P0 — Completed ✅
- ✅ Session recording/playback (transcript-synced seek + active highlight)
- ✅ Export/reporting hardening (CSV/JSON completeness, formula protection, PDF export)
- ✅ Real-event validation pass (E2E verified)
