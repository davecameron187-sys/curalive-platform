...
doc.text("Title of the Invention", { underline: true });
doc.moveDown(0.3);
para("System and Method for Artificial Intelligence-Based Anti-Hijack Detection, Journey Integrity Monitoring, Secure Vehicle Authorization, Protected Immobilization, Authenticated Reactivation, Multi-Device Correlation, Adaptive Emergency Communication, and Predictive Threat Intelligence");

doc.moveDown(1.5);
doc.fontSize(10).font("Helvetica").fillColor("#333333");
doc.text("Applicant", { underline: true });
doc.moveDown(0.3);
...
const definitions = [
  ...
  ["Threat Score", "A computed risk value derived from one or more contextual signals and used to classify the likelihood or severity of distress, hijack, tamper, theft, coercion, or unauthorized activity."],
  ...
  ["Trusted Credential", "A key transponder, OEM immobilizer credential, secure token, paired handset, paired wearable, cryptographic secret, server-issued authorization code, or other credential recognised as satisfying at least part of an authorization condition."],
...
para("Vehicle hijacking, coercive vehicle diversion, vehicle theft, signal interference, and unauthorized startup remain significant safety and asset-protection risks in many regions.");
doc.moveDown(0.3);
para("Conventional vehicle tracking systems typically rely on GPS and cellular communication, often require manual panic-button activation, and frequently become less effective when a driver cannot safely interact with a device or when a communication jammer is present.");
doc.moveDown(0.3);
...
checkPage(80);
subheading("12. Learning Feedback and Model Update (refer FIG 4)");
para("Behavioural models may be updated over time using additional journey history, verified false positives, verified incidents, operator feedback, or trusted-contact confirmation. Updated behavioural models may improve classification accuracy and reduce false positives.");
...
checkPage(200);
subheading("14. Tamper-Resilient Evidence Preservation Module (refer FIG 13)");
...
checkPage(120);
subheading("15. Geofenced Behavioural Anomaly Engine");
...
doc.addPage();
heading("Drawings", 18);
para("Figures 1 to 13 are attached hereto and form part of this specification.");
doc.moveDown(1);
