...
import { MessageCircle, X, Send, Loader2, AlertTriangle, ChevronDown, MapPin } from "lucide-react";
...
const PAGE_CONTEXT: Record<string, PageCtx> = {
  "/shadow-mode": {
    ...
    suggestions: [
      "How do I flag a disclosure risk?",
      "What does a sentiment shift mean?",
      "How do I prioritise investor questions?",
      "What triggers a compliance alert?",
    ],
    autoOpen: true,
  },
  "/call-preparation": {
    ...
    suggestions: [
      "How are questions predicted?",
      "What is a difficulty score?",
      "How should I use the talking points?",
      "What are high-risk disclosure areas?",
    ],
    autoOpen: true,
  },
  "/intelligence-terminal": {
    ...
    suggestions: [
      "How is the CICI calculated?",
      "What is market reaction correlation?",
      "How is sector difficulty scored?",
      "What does avoidance rate mean?",
    ],
  },
  "/intelligence-report": {
    ...
    suggestions: [
      "How is the communication grade calculated?",
      "What triggers a risk flag?",
      "How are exec scores determined?",
  ...
  "/investor-questions": {
    ...
    suggestions: [
      ...
      "What is avoidance detection?",
      "How does the concern tracker work?",
      "What does investor sentiment mean?",
    ],
  },
  "/benchmarks": {
    label: "Benchmarks",
    ...
    suggestions: [
      ...
      "How are sector benchmarks calculated?",
      "What is a strong communication score?",
      "How does the data flywheel work?",
    ],
  },
  "/market-reaction": {
    label: "Market Reaction",
    welcome: "You're viewing market reaction correlations — communication patterns linked to market outcomes. Ask me anything.",
    suggestions: [
      "How does avoidance affect market reaction?",
      "Which topics predict positive outcomes?",
      "How is prediction confidence scored?",
      "How many events build a reliable signal?",
    ],
  },
  "/communication-index": {
    ...
    suggestions: [
      "What are the four CICI sub-indices?",
      "When should I publish a snapshot?",
      "How is the CICI calculated?",
      "Who references the CICI?",
    ],
  },
  "/bastion": {
    ...
    suggestions: [
      "What does Bastion Capital do?",
      "How does the Bastion integration work?",
      "What data does Bastion access?",
      "Who do I contact at Bastion?",
    ],
  },
  "/lumi": {
    label: "Lumi Global",
    ...
    suggestions: [
      "What does Lumi Global provide?",
      "How does Lumi integrate with CuraLive?",
      "What attendee features does Lumi enable?",
      "How do I set up Lumi for an event?",
    ],
  },
  "/archive-upload": {
    ...
    suggestions: [
      ...
      "How does archive data feed the platform?",
      "Which intelligence features use uploaded transcripts?",
...
export default function LiveQuestionBox() {
  ...
  // Auto-open on key pages (first visit only)
  useEffect(() => {
    if (location === prevLocation.current && prevLocation.current !== "") return;
    prevLocation.current = location;

    const pageCtx = getPageCtx(location);
    if (pageCtx.autoOpen && !hasAutoOpened(location) && messages.length === 0) {
      const timer = setTimeout(() => {
        setOpen(true);
        markAutoOpened(location);
      }, 1200);
  ...
  const send = useCallback((text?: string) => {
    ...
    setInput("");
    ask.mutate({ message: msg, conversationId, currentPage: location });
  }, [input, ask, conversationId, location]);
        ...
        {open && (
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.07] bg-[#0a0c12]">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {ctx.label && (
                      <>
                        ...
                        <div className="flex items-center gap-1">
                          <MapPin className="w-2.5 h-2.5 text-violet-400/60" />
                          <span className="text-[10px] text-violet-400/70 font-medium">{ctx.label}</span>
                        </div>
                      </>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-1">
                {messages.length > 0 && (
                  <button onClick={clearHistory} className="text-[10px] text-slate-600 hover:text-slate-400 px-2 py-1 rounded transition-colors">
                    Clear
                  </button>
                )}
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
                ...
                <div className="space-y-3">
                  ...
                  <div className="pl-8 space-y-1.5">
                    <p className="text-[10px] text-slate-700 mb-2">
                      {ctx.label ? `Questions for this page:` : "Try asking:"}
                    </p>
                    {suggestions.map(s => (
                      <button
                        key={s}
              ...
              {/* When there ARE messages, show quick-reply chips at top */}
              {messages.length > 0 && (
            ...
            <div className="px-3 py-3 border-t border-white/[0.07] bg-[#0a0c12]">
              <div className="flex items-center gap-2 bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2 focus-within:border-violet-500/40 transition-colors">
                <input
                  ...
                  placeholder={ctx.label ? `Ask about ${ctx.label}…` : "Ask about CuraLive…"}
