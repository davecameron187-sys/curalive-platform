...
export default function OCC() {
  ...
  // Audio library queries & mutations
  const audioFilesQuery = trpc.occ.getAudioFiles.useQuery(
    { conferenceId: activeCCPConferenceId ?? 0 },
    { enabled: !!activeCCPConferenceId, retry: false }
  );
  const addAudioFileMut = trpc.occ.addAudioFile.useMutation({
    onSuccess: () => { audioFilesQuery.refetch(); setShowAudioUpload(false); setAudioUploadName(""); },
    onError: (e) => toast.error(`Failed to save audio file: ${e.message}`),
  });
  const deleteAudioFileMut = trpc.occ.deleteAudioFile.useMutation({
    onSuccess: () => audioFilesQuery.refetch(),
    onError: (e) => toast.error(`Failed to delete audio file: ${e.message}`),
  });
  const setAudioPlayStateMut = trpc.occ.setAudioPlayState.useMutation();

  const handleAudioPlay = (file: { id: number; fileUrl: string }) => {
    if (playingAudioId === file.id) {
      audioElemRef.current?.pause();
      setPlayingAudioId(null);
      setAudioPlayStateMut.mutate({ id: file.id, isPlaying: false });
    } else {
      ...
      audio.play().catch(() => {});
      audio.onended = () => { setPlayingAudioId(null); setAudioPlayStateMut.mutate({ id: file.id, isPlaying: false }); };
      setPlayingAudioId(file.id);
      setAudioPlayStateMut.mutate({ id: file.id, isPlaying: true });
    }
  };

  const handleAudioUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file || !activeCCPConferenceId) return;
    ...
    try {
      ...
      await addAudioFileMut.mutateAsync({
        conferenceId: activeCCPConferenceId,
        name: audioUploadName.trim() || file.name.replace(/\.[^.]+$/, ""),
        fileUrl: url,
        fileKey: key,
      });
      ...
      <div className="flex items-center justify-between px-3 py-1 bg-[#0c1220] border-b border-slate-700/60 shrink-0" style={{ background: "linear-gradient(180deg, #0e1628 0%, #0a0f1e 100%)" }}>
        ...
        <div className="flex items-center gap-1">
          {[
            ...
            { icon: GraduationCap, label: "Training", count: null, color: "text-violet-400", onClick: () => navigate("/operator-hub") },
              ...
              <>
                ...
                <div className="border-t border-slate-700 shrink-0">
                  ...
                  <div className="p-3 bg-[#0d1526]" style={{ minHeight: "120px" }}>
                    ...
                    {/* Audio Files */}
                    {featureTab === "audio" && (
                      <div className="flex flex-col gap-2" style={{ height: "180px" }}>
                        <div className="flex items-center justify-between shrink-0">
                          <span className="text-xs text-slate-400">Pre-recorded audio files</span>
                          <button
                            onClick={() => setShowAudioUpload(v => !v)}
                            className="flex items-center gap-1 px-2 py-0.5 text-[10px] bg-blue-700/40 hover:bg-blue-700 text-blue-300 rounded transition-colors"
                          >
                            <Upload className="w-3 h-3" />Upload
                          </button>
                        </div>
                        {showAudioUpload && (
                          <div className="shrink-0 flex flex-col gap-1 p-2 bg-slate-800 rounded border border-slate-700">
                            <input
                              type="text"
                              value={audioUploadName}
                              onChange={e => setAudioUploadName(e.target.value)}
                              placeholder="File label (optional)"
                              className="bg-slate-700 border border-slate-600 rounded px-2 py-1 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500"
                            />
                            <label className={`flex items-center gap-1.5 px-2 py-1 rounded text-[10px] cursor-pointer transition-colors ${audioUploading ? "bg-slate-700 text-slate-500" : "bg-emerald-700/40 hover:bg-emerald-700 text-emerald-300"}`}>
                              {audioUploading ? "Uploading…" : <><Upload className="w-3 h-3" />Choose MP3 / WAV</>}
                              <input
                                type="file"
                        ...
                        <div className="flex-1 overflow-y-auto">
                          {audioFilesQuery.isLoading && (
                            <div className="text-[10px] text-slate-500 py-2">Loading…</div>
                          )}
                          {!audioFilesQuery.isLoading && (audioFilesQuery.data ?? []).length === 0 && (
                            <div className="text-[10px] text-slate-600 py-2">No audio files yet — upload an MP3 or WAV above.</div>
                          )}
                          {(audioFilesQuery.data ?? []).map(f => (
                            <div key={f.id} className="flex items-center justify-between py-1.5 border-b border-slate-800">
                              <div className="flex items-center gap-2 min-w-0">
                                <Music className={`w-3.5 h-3.5 shrink-0 ${playingAudioId === f.id ? "text-emerald-400" : "text-slate-500"}`} />
                              ...
                              <div className="flex gap-1 shrink-0">
                                <button
                                  onClick={() => handleAudioPlay(f)}
                                  className={`p-1 rounded transition-colors ${playingAudioId === f.id ? "bg-amber-700/60 hover:bg-amber-700 text-amber-400" : "bg-emerald-700/40 hover:bg-emerald-700 text-emerald-400"}`}
                                >
                                  {playingAudioId === f.id ? <PauseCircle className="w-3.5 h-3.5" /> : <PlayCircle className="w-3.5 h-3.5" />}
                                </button>
                                <button
                                  onClick={() => { if (confirm(`Delete "${f.name}"?`)) deleteAudioFileMut.mutate({ id: f.id }); }}
                                  className="p-1 rounded bg-red-900/40 hover:bg-red-800 text-red-400 transition-colors"
                                >
